...
export function debounce<T extends any[]>(func: Func<T>, wait: number): Func<T> {
    ...
    return function debounced(this: any, ...args: T) {
...
export function toFixed(amount: number | string, fix: number, mode: 'reg' | 'fixed' = 'fixed'): string | number {
    const num = Number(amount);

    if (mode === 'reg') {
        const reg = new RegExp(`^\\d+(?:\\.\\d{0,${fix}})?`);
        const newNum = String(amount).match(reg);
        const newAmount = newNum ? newNum[0] : '0';

        const [integer, decimal] = newAmount.split('.');
        const padDecimal = decimal ? decimal.padEnd(fix, '0') : '0'.repeat(fix);
        return `${integer}.${padDecimal}`;
    }
    ...
    const newNum = num.toFixed(fix + 1);
    const newAmount = newNum.slice(0, -1);
    return Number(newAmount);
}

// form input 验证只能输入数字
// export const handleNumberInput = (event: React.ChangeEvent<HTMLInputElement>, form: FormInstance, fieldName: string) => {
//     const value = event.target.value;
//     const modifiedValue = value.replace(/[^0-9]/g, ''); // 只允许输入数字
//
//     form.setFieldsValue({
...
// form input 验证只能输入数字或大小写字母
// export const handleLetterInput = (event: React.ChangeEvent<HTMLInputElement>, form: FormInstance, fieldName: string) => {
//     const value = event.target.value;
//     const modifiedValue = value.replace(/[^a-zA-Z0-9]/g, ''); // 只允许输入数字
//
//     form.setFieldsValue({
//         [fieldName]: modifiedValue,
//     });
...
export function uniqueBy<T, K extends keyof T>(arr: T[], key: K): T[] { // 数组去重
    return arr.reduce((result: T[], item: T) => {
        const value = item[key];
        if (!result.find((i) => i[key] === value)) {
            result.push(item);
        }
        return result;
    }, []);
...
export function handleCopyText(val: string) {
    try {
        copyToClipboard(val);
    } catch (e: any) {
        throw new Error(e);
